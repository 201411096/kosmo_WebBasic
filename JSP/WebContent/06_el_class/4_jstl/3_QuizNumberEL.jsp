...
<!-- 세션이 유지되는 동암 c var인 answer를 유지함  -->
정답 : ${answer}

<!-- 4. guess의 변수에 값이 있다면 -->
<!--  (1) guess와 answer값이 같다면 맞춰다고 출력 -->
<%-- <c:if test="${answer eq param.guess }"> --%>
<!-- 	<br> -->
<!-- 	정답입니다. -->
<%-- </c:if> --%>
<!--  (2) 다르다면 guess값이 answer값보다 크면 크다고 출력 작으면 작다고 출력 -->
<c:choose>
	<c:when test="${answer lt param.guess }">
		정답은 입력한 값보다 작은 수 입니다.
	</c:when>
	<c:when test="${answer gt param.guess }">
		정답은 입력한 값보다 큰 수 입니다.
	</c:when>
	<c:when test="${answer eq param.guess }">
		정답입니다.
	</c:when>
</c:choose>
