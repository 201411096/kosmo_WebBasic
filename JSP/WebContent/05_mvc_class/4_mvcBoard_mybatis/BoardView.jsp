...
// 	BoardRec rec = service.getArticleById(articleId);
	Board rec = (Board)request.getAttribute("boardRec");
%>    
<!DOCTYPE html>
<html>
<head>
...
<%-- 			<a href="BoardReplyForm.jsp?parentId=<%=rec.getArticleId()%>">답변하기</a> --%>
			<a href="<%=projectName %>/BoardMyBatisControl?cmd=reply-form&parentId=<%=rec.getArticleId()%>">답변하기</a>
			<a href="<%=projectName %>/BoardMyBatisControl?cmd=update-form&id=<%=rec.getArticleId()%>">수정하기</a>
			<a href="<%=projectName %>/BoardMyBatisControl?cmd=delete-form&id=<%=rec.getArticleId()%>">삭제하기</a> 	
		</td>
	</tr>
	</table>
